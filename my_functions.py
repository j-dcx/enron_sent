...
CSV = 'data/data_users_as_labels-sent_'
...
def plot_scores(GLOBAL_PATH, model_language, graph_data, user_input_features_df, metric):
    with open(GLOBAL_PATH + model_language + '/' + GRAPH_DATA_DIRECTORY + graph_data, 'rb') as f:
...
def load_dataset(GLOBAL_PATH, model_language):
    return pd.read_csv(GLOBAL_PATH + model_language + '/' + CSV + model_language + '.csv')
